#!/usr/bin/env python3
"""
test_fungibility_matrix_mem.py
==============================
Runs a matrix experiment to prove the fungibility of the Memory (RAM) "unit of measure".
It tests combinations of Background Memory intensity and Probe Memory Mode under a mixed 
Memory background workload, proving that the Normalized App Footprint calculation 
...
# The 3 memory modes
PROBE_MODES = ["mem_copy", "mem_read", "mem_write"]
...
def plot_matrix(csv_path, out_plot):
    ...
    ax.set_xlabel("Background Memory Workload Intensity", fontsize=11, fontweight="bold")
    ax.set_title("Memory Unit of Measure Fungibility:\nFootprint Measurement Invariance Across Different Probes and Intensities", 
...
def main():
    parser = argparse.ArgumentParser(description="Test Memory Fungibility Matrix with mixed background at varying intensities.")
    parser.add_argument("--bg-procs", type=int, default=8)
    parser.add_argument("--bg-io-mix", type=float, default=0.0) 
    parser.add_argument("--bg-mem-mix", type=float, default=1.0) # Memory tests default to 1.0 memory mix
    parser.add_argument("--bg-intensities", type=str, default="0.2,0.4,0.6,0.8",
                        help="comma-separated list of background memory intensities to sweep (default: 0.2,0.4,0.6,0.8)")
    parser.add_argument("--duration", type=int, default=60)
    parser.add_argument("--out-dir", type=str, default="results/fungibility_matrix_mem")
    ...
    if not args.only_plot:
        ...
        print("\n" + "="*60)
        print(" Phase 2: Probe Slack for Memory Fungibility Matrix")
        print("="*60)
        ...
        for intensity in intensities:
            ...
            for probe_mode in PROBE_MODES:
                ...
                env["BG_MEM_MODE"] = "mixed"
                env["PROBE_MEM_MODE"] = probe_mode

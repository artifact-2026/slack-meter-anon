...
def run_probe(
    ...
    """Run bg + probe workers concurrently samples times; return average (bg_tput, probe_tput) in ops/s."""
...
def sweep(
    ...
    if n_baseline > 1:
        print(f"  Baseline bg : {baseline_tput:,.0f} Tokens/s (mean of {n_baseline})")
    else:
        print(f"  Baseline bg : {baseline_tput:,.0f} Tokens/s")
    print(f"  Threshold   : {threshold:,.0f} Tokens/s  (drop >= {drop_pct*100:.1f}%)")
    ...
    def _run_phase2(n_full_: int, seed_ktokens: float) -> tuple[list[dict], float, float]:
        ...
        for step in range(1, binary_steps + 1):
            ...
            print(f"  {step:>4d}  {mid:>9.3f}  {bg2:>12.0f}  {p2:>12.0f}  {s2}")
    ...
    while n_probe <= max_probes:
        ...
        print(f"  {n_probe:>7d}  {bg_tput:>12.0f}  {probe_tput:>12.0f}  {status}")
...
def main() -> None:
    ...
    parser.add_argument("--drop-pct",     type=float, default=0.10,  metavar="F")
    ...
    print("  Result")
    print("=" * 60)
    print(f"  Baseline bg throughput : {result['baseline_bg_ktokens']*1000:.0f} Tokens/s")
